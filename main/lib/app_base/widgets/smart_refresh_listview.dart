      ...
      footer: CustomFooter(
        builder: (BuildContext context, LoadStatus? mode) {
          ...
          if (mode == LoadStatus.idle) {
            body = Text('上拉加載更多', style: TextStyle(color: widget.fontColor));
          } else if (mode == LoadStatus.loading) {
            body = const CupertinoActivityIndicator();
          } else if (mode == LoadStatus.failed) {
            body = Text('加載失敗，點擊重試', style: TextStyle(color: widget.fontColor));
          } else if (mode == LoadStatus.canLoading) {
            body = Text('鬆開手指進行加載', style: TextStyle(color: widget.fontColor));
          } else {
            body = Text('已經到底了', style: TextStyle(color: widget.fontColor));
          }
          return SizedBox(
            height: 55,
            child: Container(
